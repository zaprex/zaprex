...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered zaprex address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ZPX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal zaprex:URI (zaprex:XyZ...?message=example)
            {
...
static void NotifyzZPXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzZPXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzZPXReset.connect(boost::bind(NotifyzZPXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzZPXReset.disconnect(boost::bind(NotifyzZPXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zzpxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
