...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZPX);
    ...
    if (!settings.contains("nAnonymizeZaprexAmount"))
        settings.setValue("nAnonymizeZaprexAmount", 1000);

    nAnonymizeZaprexAmount = settings.value("nAnonymizeZaprexAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZaprexAmount"))
        SoftSetArg("-anonymizezaprexamount", settings.value("nAnonymizeZaprexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zaprex.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZaprexAmount:
            return QVariant(nAnonymizeZaprexAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZaprexAmount:
            nAnonymizeZaprexAmount = value.toInt();
            settings.setValue("nAnonymizeZaprexAmount", nAnonymizeZaprexAmount);
            emit anonymizeZaprexAmountChanged(nAnonymizeZaprexAmount);
            break;
        case CoinControlFeatures:
