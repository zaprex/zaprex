...
CzZPXWallet::CzZPXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zzpx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZPXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZPXSeed_deprecated()) {
                    LogPrintf("%s: Updated zZPX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zzpx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zzpx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzZPXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZPXCount(nCountLastUsed);
    else if (!walletdb.ReadZZPXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZPXWallet::Lock()
{
    seedMaster = 0;
}

void CzZPXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZPXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZPX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZPXWallet::LoadMintPoolFromDB()
{
...
void CzZPXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZPXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZPXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zzpxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzZPXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZPX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zzpxTracker which also adds to database
    pwalletMain->zzpxTracker->Add(dMint, true);
...
void CzZPXWallet::SeedToZZPX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZPXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZPXWallet::GenerateDeterministicZZPX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZPXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZPXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
