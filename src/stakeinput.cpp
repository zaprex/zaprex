...
// The zZPX block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zZPX is a hash of the serial
...
{
    //Create an output returning the zZPX that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZZPXOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zZPX output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zZPX", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZZPXOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zZPX output", __func__);
...
{
    CzZPXTracker* zzpxTracker = pwallet->zzpxTracker.get();
    CMintMeta meta;
    if (!zzpxTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zzpxTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ZPX stake is the outpoint
