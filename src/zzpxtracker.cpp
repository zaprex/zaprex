...
CzZPXTracker::CzZPXTracker(std::string strWalletFile)
{
...
CzZPXTracker::~CzZPXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzZPXTracker::Init()
{
...
bool CzZPXTracker::Archive(CMintMeta& meta)
{
...
bool CzZPXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzZPXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzZPXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzZPXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzZPXTracker::GetSerialHashes()
{
...
CAmount CzZPXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzZPXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzZPXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzZPXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzZPXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzZPXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzZPXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzZPXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzZPXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzZPXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzZPXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzZPXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzZPXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzZPXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzZPXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzZPXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzZPXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzzpx from DB\n", __func__, listDeterministicDB.size());
    }
