...
#ifndef Zaprex_ZZPXWALLET_H
#define Zaprex_ZZPXWALLET_H
...
public:
    CzZPXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZZPX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZZPX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Zaprex_ZZPXWALLET_H
